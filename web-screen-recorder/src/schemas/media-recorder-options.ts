...
type M = ConstructorParameters<typeof MediaRecorder>;
...
export const MediaRecorderOptions_ = z
	.object({
		...
		videoBitsPerSecond: z.enum([
			...
			"20000000"  // 20Mbps - 4K
		]).describe("비디오 비트 속도 (bps). 낮은 값(1Mbps)은 저화질/모바일에 적합하고, 높은 값(20Mbps)은 4K 고화질 녹화에 적합함. " + 
			"이 값은 비디오 스트림에만 적용되며, 미설정 시 bitsPerSecond 값이 비디오 스트림의 기본값으로 사용됨. " + 
			"단, bitsPerSecond가 설정된 경우 모든 스트림의 총합은 이 값을 초과할 수 없음"),
		bitsPerSecond: z.enum([
			"1500000",  // 1.5Mbps - 기본 화질
			"3000000",  // 3Mbps - 중간 화질
			"6000000",  // 6Mbps - 고화질
			"10000000", // 10Mbps - Full HD
			"15000000"  // 15Mbps - 4K
		]).describe("전체 미디어의 총 비트 속도 제한값(상한선). audioBitsPerSecond와 videoBitsPerSecond를 모두 설정한 경우에도 " + 
